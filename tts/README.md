# Bayesian Parameter-Efficient Fine-Tuning for Overcoming Catastrophic Forgetting: speech synthesis experiments with StyleTTS 2

## Setup

1. Follow the instructions from the original repository to setup the python environment and download the pre-trained model.
2. Install Hugging Face [PEFT](https://github.com/huggingface/peft).
3. Download VCTK and LibriTTS and point `data_params:root_path` to the corresponding dataset root directory in the config files.

## Hessian estimation

```shell
python train_finetune_peft.py \
--config_path Configs/config_ft_compute_hessian_lt1000.yml \
--log_dir $HESSIAN_OUTPUT_DIR \
--lora_r 16 \
--lora_alpha 32 \
--hessian_method {kfac,ewc} \
--compute_hessian
```

## Fine-tuning

For fine-tuning with LoRA+KFAC/EWC:

```shell
python train_finetune_peft.py \
--config_path Configs/config_ft_p248.yml \
--log_dir $OUTPUT_DIR \
--lora_r 16 \
--lora_alpha 32 \
--hessian_method {kfac,ewc} \
--hessian_lambda 1e3 \
--lora_path $HESSIAN_OUTPUT_DIR/epoch_2nd_00000.pth
```
...
For fine-tuning linear layers:

```shell
python train_finetune_linear.py \
--config_path Configs/config_ft_p248_linear.yml \
--log_dir $OUTPUT_DIR
```
...
Refer to Inference_LibriTTS_peft.ipynb for LoRA models or Inference_LibriTTS.ipynb for fine-tuning linear layers or full fine-tuning.
